## Changelog - GPT session 2025-07-28 Part 2 ##
📦 Databaseændringer:
Tabel oprettet: day_mission_logs

Formål: Logger, hvornår en bruger fuldfører en ugentlig mission.
...
Kontrollere om en mission er blevet fuldført og skjule widget indtil næste mandag kl. 04:00.

Dokumentere mission-historik for hver bruger.

Supabase RPC-funktion: get_random_success_message()

Returnerer en tilfældig motiverende besked.

Bruges når en mission fuldføres, så beskeden er varieret og positiv.

Tabel anvendt: dashboard_images

Bruges til at hente seneste billede uploadet i dashboardet (minder).

Hvis der ikke findes billeder fra de seneste 7 dage, vises fallback-tekst.

🎨 Frontendændringer i WeeklyMissionCard.tsx
✅ Funktioner implementeret:
Visning af ugentlig mission:

Teksten hentes fra day_missions med created_at > mandag kl. 04:00.

Visningen er sløret indtil man trykker “🔓 Vis mission”.

Mission Fuldført Flow:

Ved klik på “Fuldfør mission”:

Data indsættes i day_mission_logs.

Success-besked vises (fra get_random_success_message eller fallback).

Et billede fra dashboard_images (seneste 7 dage) vises.

Hvis intet billede findes: fallback-tekst vises.

Countdown på 15 sekunder starter.

Derefter redirect til /dashboard.

Successmeddelelse og billedevisning:

🎉 Dynamisk besked vises med countdown.

 fundet:

Vis billede med tekst:

Dette var jeres sidste oplevelse som skabte et minde for jeres parforhold. Er der sket noget nyt siden, så sæt pris på jeres oplevelser og dokumentér det nu.

CTA: [Upload] eller [Nej tak]

Hvis ingen billede fundet:

Vis tekst:

Har I haft nogle oplevelser den seneste uge som gav jer en oplevelse?

Skjuler hele widget’en hvis mission allerede er fuldført (baseret på localStorage) – med undtagelse af TEST_MODE.

🧪 Testtilstand (midlertidig)
TEST_MODE = true betyder, at mission vises uanset om man har fuldført.

Bruges til udvikling og test uden at widget skjules.

📝 Husk ved næste session:
day_mission_logs skal bruges til statistik, tracking og fremtidig funktionalitet.

Success-message håndtering skal altid inkluderes.

Visuelle og motiverende elementer er centrale i widget flowet.

CTA'erne "Upload" og "Nej tak" kan senere bindes op på logik eller statistik.

## Changelog - GPT session 2025-07-28 ##
...
Bekræftet at begge brugeres svar blev fundet korrekt i konsollen.
