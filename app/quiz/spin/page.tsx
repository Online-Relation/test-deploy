...
const colors = [
  '#6B46C1', // stærk lilla (felt 1)
  '#805AD5',
  '#9F7AEA',
  '#B794F4',
  '#6B46C1', // stærk lilla (felt 5)
  '#805AD5',
  '#9F7AEA',
  '#B794F4',
];
...
export default function QuizSpin() {
  ...
  const spinWheel = () => {
    ...
    const extraSpins = 5; // antal hele omgange
    const randomSegment = Math.floor(Math.random() * SEGMENT_COUNT);

    // Beregn rotation så pilen peger på midten af segmentet (roterer mod uret)
    const finalRotation =
    ...
    setTimeout(() => {
      setIsSpinning(false);
    }, 5000); // skal matche transition varighed
  };

  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-pink-50 to-purple-100 p-6">
      <h1 className="text-3xl font-bold mb-8 text-indigo-700">Hvem starter samtalen?</h1>

      <div className="relative w-64 h-64">
  {/* Pil - fast position, peger nedad */}
  <div
  style={{
    position: 'absolute',
    top: '-10px',
    left: '50%',
    transform: 'translateX(-50%) rotate(180deg)', // Roteret 180 grader
    width: 0,
    height: 0,
    borderLeft: '16px solid transparent',
    borderRight: '16px solid transparent',
    borderBottom: '28px solid #4C51BF',
    zIndex: 10,
...
>
      ...
      >
        {isSpinning ? 'Spinner...' : 'Spin hjulet'}
      </button>
