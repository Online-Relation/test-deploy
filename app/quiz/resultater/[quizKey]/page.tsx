...
interface Question { id: string; question: string; type: string; order: number }
interface Answer { question_id: string; answer: string; user_id: string; session_id: string; status: string }
...
export default function QuizResultPage() {
  ...
  const [grouped, setGrouped] = useState<{ green: Question[]; yellow: Question[]; red: Question[] } | null>(null);
  const [answers, setAnswers] = useState<Answer[]>([]);
  const [loading, setLoading] = useState(false);

  // Hent samtalestarter for NETOP DENNE SESSION
  useEffect(() => {
    const fetchStarterName = async () => {
      if (!sessionId) { setStarterName(null); return; }
      ...
      if (error) { console.error('Fejl ved hentning af samtalestarter:', error); setStarterName(null); return; }
      ...
      if (data?.user_id) {
        const { data: profile, error: profileError } = await supabase
          ...
          .eq('id', data.user_id)
          .maybeSingle();
        if (profileError) { console.error('Fejl ved hentning af profilnavn:', profileError); setStarterName(null); return; }
  ...
  useEffect(() => {
    const fetchEverything = async () => {
      ...
      // Tildel XP til alle submitted for denne session (rollebaseret)
      if (answersData && answersData.length > 0) {
        ...
        for (const userId of uniqueUserIds) {
          ...
          const role = userProfile?.role || 'mads';
          await awardQuizXpToUser({ userId, quizKey, effort, role });
        }
      }
      ...
      if (!questions || !answersData) { setLoading(false); return; }
      setAnswers(answersData as any);

      const importanceScale = ['ikke vigtigt','mindre vigtigt','vigtigt','meget vigtigt'];
      const result = { green: [] as Question[], yellow: [] as Question[], red: [] as Question[] };

      for (const q of questions) {
        const related = (answersData || []).filter((a: any) => a.question_id === q.id);
        const uniqueUsers = [...new Set(related.map((a: any) => a.user_id))];
        if (uniqueUsers.length !== 2) continue;
        let [a1, a2] = related.map((a: any) => a.answer.trim().toLowerCase().replace(/[.,!?]/g, ''));
        if (a1 === a2) { result.green.push(q); continue; }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-8">
      ...
      {loading ? (
        ...
        <>
          <QuizResultComponent grouped={grouped} answers={answers} sessionId={sessionId} quizKey={quizKey} />
          {recommendation && (
